        ...
        try {
            ...
            mOmniDriver.init(object : OmniConnection {
                override fun onConnected() {
                    result = "initOmniDriver--->onSuccess"
                    isInitSuccess = true
                    Toast.makeText(this@MainActivity, result, Toast.LENGTH_SHORT).show()
                }

                override fun onDisconnected(error: Int) {
                    result = "initOmniDriver--->onError:error=$error"
                    isInitSuccess = false
                    Toast.makeText(this@MainActivity, result, Toast.LENGTH_SHORT).show()
                    if (error == OmniDriver.ERROR_BIND_FAIL) {
                        Toast.makeText(this@MainActivity, "Please confirm whether it is installed OmniDriver service", Toast.LENGTH_SHORT).show()
                        return
                    }
                    if (retryCount < 3) {
                        retryCount++
